...
import com.example.product.domain.InventoryRequest;
...
public class ProductResource {
    ...
    public ResponseEntity<ProductDTO> createProduct(@RequestBody ProductDTO productDTO) throws URISyntaxException {
        ...
        // Create the request body for the inventory service
        InventoryRequest inventoryRequest = new InventoryRequest();
        inventoryRequest.setProductId(productDTO.getId());
        inventoryRequest.setQuantity(1);
        inventoryRequest.setUpdatedAt(Instant.now());

        // Make the request to the inventory service
        RestTemplate restTemplate = new RestTemplate();
        String inventoryServiceUrl = "http://localhost:8080/api/inventories";
        try {
            ...
            LOG.debug("Successfully reserved 1 unit in inventory for product {}", productDTO.getId());
        } catch (Exception e) {
            LOG.error("Failed to reserve inventory for product {}", productDTO.getId(), e);
